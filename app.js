(() => {
  ...
  const CONFIG = {
    mode: "backward",
    practiceDigits: ["274", "589", "603"], // 3桁×3本（固定）
    main: {
      minDigits: 3,
      maxDigits: 9,
      trialsPerLevel: 9,
  ...
  async function warmupAudio() {
    ...
    if (CONFIG.audio.beep) tasks.push(ensureAudioCached(CONFIG.audio.beep));
  ...
  async function nextTrial() {
    ...
    if (CONFIG.audio.beep) await playAudio(CONFIG.audio.beep);
    ...
    updateStatus("逆順で入力してください。Enterで送信。", mode);
  }
  ...
  function handleSubmit() {
    ...
    const expected = trial.digits.split("").reverse().join("");
  ...
  function downloadResults() {
    ...
    a.href = url;
    a.download = `digitspan_backward_${now}.csv`;
    a.click();
